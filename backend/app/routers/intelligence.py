...
async def proxy_check(target: str = "https://api.ipify.org?format=json"):
    ...
    try:
        pm = ProxyManager(settings.PROXIES or "", settings.PROXY_USERNAME, settings.PROXY_PASSWORD)
        if not pm.has_proxies():
            return {"using_proxies": False, "message": "No proxies configured"}

        samples = []
        # Try up to 3 samples (or number of proxies available)
        for _ in range(min(3, len((settings.PROXIES or '').split(',')))):
